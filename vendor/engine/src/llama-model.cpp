...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    if (split_sum > 0.0f) {
        ...
        // Every device reported zero free memory. The draft half of a
        // speculative pair is the way in: the target loads first and can take
        // all of a small card, and a device with nothing left reports free = 0
        // against a real total, which the fallback above does not catch.
        //
        // The division is what matters, not the split. Zero over zero is NaN,
        // every comparison against NaN is false, and upper_bound below then
        // walks off the end of `splits` and indexes `devices` one past its
        // last - which throws out of load_tensors as "invalid vector
        // subscript", naming neither the device nor the memory that ran out.
        // An even split is the only meaningful answer left and it keeps the
        // points finite.
        for (size_t i = 0; i < n_devices(); ++i) {
    ...
    auto get_layer_buft_list = [&](int il) -> llama_model::impl::layer_dev {
        ...
        // Clamped because this indexes `devices`: the split points are meant
        // to end at 1.0 and a query below that lands inside, but a rounding
        // edge should cost the last device rather than an exception thrown
        // from the middle of loading a model.

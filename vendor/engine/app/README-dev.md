# gguf-server Development Documentation

This document provides an in-depth technical overview of `gguf-server`, intended for maintainers and contributors.

If you are an end user consuming `gguf-server` as a product, please refer to the main [README](./README.md) instead.
...
`gguf-server` includes an automated test suite based on `pytest`.

The framework automatically starts a `gguf-server` instance, sends requests, and validates responses.
...
- A new `gguf-server` subprocess will be spawned with special `SERVER_CHILD_MODE_DOWNLOAD`
...
The project includes a web-based user interface for interacting with `gguf-server`. It supports both single-model (`MODEL` mode) and multi-model (`ROUTER` mode) operation.
...
After `public/index.html` has been generated, rebuild `gguf-server` as described in the [build](#build) section to include the updated UI.

**Note:** The Vite dev server automatically proxies API requests to `http://localhost:8888`. Make sure `gguf-server` is running on that port during development.

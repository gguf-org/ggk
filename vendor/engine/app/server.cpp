...
// satisfies -Wmissing-declarations (used by the gk command)
int gk_server(int argc, char ** argv);

// to be used via CLI (argc / argv are used by router mode only)
int gk_server(common_params & params, int argc, char ** argv);
void gk_server_terminate();
void gk_server_terminate() {
    if (shutdown_handler) {
        shutdown_handler(0);
    }
...
int gk_server(int argc, char ** argv) {
    ...
    return gk_server(params, argc, argv);
}

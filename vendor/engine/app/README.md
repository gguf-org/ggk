...
| `--port PORT` | port to listen (default: 8888)<br/>(env: LLAMA_ARG_PORT) |
...
`gguf-server` is built alongside everything else from the root of the project

- Using `CMake`:

  ```bash
  cmake -B build
  cmake --build build --config Release -t gguf-server
  ```

  Binary is at `./build/bin/gguf-server`

## Build with SSL

`gguf-server` can also be built with SSL support using OpenSSL 3
...
```bash
./gguf-server -m models/7B/ggml-model.gguf -c 2048
```

### Windows

```powershell
gguf-server.exe -m models\7B\ggml-model.gguf -c 2048
```

The above command will start a server that by default listens on `127.0.0.1:8888`.
You can consume the endpoints with Postman or NodeJS with axios library. You can visit the web front end at the same url.

### Docker

```bash
docker run -p 8888:8888 -v /path/to/models:/models ghcr.io/ggml-org/llama.cpp:server -m models/7B/ggml-model.gguf -c 512 --host 0.0.0.0 --port 8888

# or, with CUDA:
docker run -p 8888:8888 -v /path/to/models:/models --gpus all ghcr.io/ggml-org/llama.cpp:server-cuda -m models/7B/ggml-model.gguf -c 512 --host 0.0.0.0 --port 8888 --n-gpu-layers 99
```
...
```shell
curl http://localhost:8888/v1/messages/count_tokens \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4",
    "messages": [
...
`gguf-server` can be launched in a **router mode** that exposes an API for dynamically loading and unloading models. The main process (the "router") automatically forwards each request to the appropriate model instance.

To start in router mode, launch `gguf-server` **without specifying any model**:
...
```sh
gguf-server --models-dir ./models_directory
```
...
```sh
gguf-server -ctx 8192 -n 1024 -np 2
```
...
```sh
gguf-server --models-preset ./my-models.ini
```
...
1. **Command-line arguments** passed to `gguf-server` (highest priority)
...
`gguf-server` returns errors in the same format as OAI: https://github.com/openai/openai-openapi
...
```bash
./gguf-server -m model.gguf --ui-config '{"pasteLongTextToFileLen": 0, "renderUserContentAsMarkdown": true}'
```

...
// zlib decoder from stb_image (zlib-only, file-local implementation)
...
namespace {
    ...
    bool decompress_zlib(const uint8_t* data,
        ...
        int out_len   = 0;
        char* decoded = stbi_zlib_decode_malloc(reinterpret_cast<const char*>(data),
        ...
        text.assign(decoded, static_cast<size_t>(out_len));
        free(decoded);
        return true;
    }

...
void print_usage(int argc, const char* argv[], const std::vector<ArgOptions>& options_list) {
    ...
    // Handled before the parser runs, so they are not in any option list.
    std::cout << "\nInformational (must be the first argument):\n";
    std::cout << "  --version                          print the engine version and exit\n";
    std::cout << "  --list-devices                     print the compute devices --backend can\n"
                 "                                     address, as JSON, and exit\n";
}
...
bool save_results(const SDCliParams& cli_params,
    ...
    if (results == nullptr || num_results <= 0) {
        ...
        // audio-only generation (ACE-Step): the wav is the primary output
        namespace fs      = std::filesystem;
        fs::path out_path = cli_params.output_path;
        ...
        fs::path wav_path     = out_path;
        std::string ext_lower = wav_path.has_extension() ? wav_path.extension().string() : "";
        std::transform(ext_lower.begin(), ext_lower.end(), ext_lower.begin(), ::tolower);
        if (ext_lower != ".wav") {
        ...
        LOG_ERROR("failed to save result audio to '%s'", wav_path.string().c_str());
        return false;
    }
...
// What --backend and --split-mode can be pointed at, as JSON on stdout.
//
// The engine's device numbering is its own. CUDA hands out indices
// fastest-first by default, so cuda0 is frequently not nvidia-smi's GPU 0, and
// a caller that assumed otherwise silently assigns modules to the wrong card.
// Anything driving this binary should ask rather than guess, and JSON is the
// form the GUI can read without a parser of its own.
static void print_devices() {
    const int count = sd_list_devices(nullptr, 0);

    std::vector<sd_device_info_t> devices((size_t)std::max(count, 0));
    if (count > 0) {
        sd_list_devices(devices.data(), count);
    }

    std::cout << "[";
    for (int i = 0; i < count; ++i) {
        // Device descriptions come from the driver and have never contained a
        // quote or a backslash, but they are not ours to promise about.
        std::string description;
        for (const char* p = devices[(size_t)i].description; p != nullptr && *p; ++p) {
            if (*p == '"' || *p == '\\') {
                description += '\\';
            }
            description += *p;
        ...
        std::cout << (i == 0 ? "\n" : ",\n")
                  << "  {\"name\": \"" << devices[(size_t)i].name << "\""
                  << ", \"description\": \"" << description << "\""
                  << ", \"is_gpu\": " << (devices[(size_t)i].is_gpu ? "true" : "false")
                  << ", \"memory_free\": " << devices[(size_t)i].memory_free
                  << ", \"memory_total\": " << devices[(size_t)i].memory_total
                  << "}";
    }
    std::cout << (count > 0 ? "\n]" : "]") << "\n";
}
...
int main(int argc, const char* argv[]) {
    ...
    // NAN marks "not set on the command line" so the model-specific default can
    // be applied after the context (and thus the model version) is known.
    gen_params.sample_params.guidance.txt_cfg            = NAN;
    gen_params.high_noise_sample_params.guidance.txt_cfg = NAN;
    ...
    if (cli_params.mode == UPSCALE) {
        ...
        if (std::isnan(gen_params.sample_params.guidance.txt_cfg)) {
            gen_params.sample_params.guidance.txt_cfg = sd_get_default_txt_cfg(sd_ctx.get());
        }
        if (std::isnan(gen_params.high_noise_sample_params.guidance.txt_cfg)) {
            gen_params.high_noise_sample_params.guidance.txt_cfg = sd_get_default_txt_cfg(sd_ctx.get());
        }
        if (!gen_params.width_and_height_are_set()) {
            int default_width  = 0;
            int default_height = 0;
            sd_get_default_resolution(sd_ctx.get(), &default_width, &default_height);
            gen_params.set_width_and_height_if_unset(default_width, default_height);
        }

        if (sd_ctx_supports_audio_generation(sd_ctx.get())) {
            // audio-only models (ACE-Step) run through the video entry point,
            // whatever mode was requested; the result comes back as audio.
            sd_vid_gen_params_t vid_gen_params = gen_params.to_sd_vid_gen_params_t();
            if (!generate_video(sd_ctx.get(), &vid_gen_params, nullptr, &num_results, &generated_audio)) {
                LOG_ERROR("generate audio failed");
                return 1;
            }
        } else if (cli_params.mode == IMG_GEN) {
        ...
        if (!results && generated_audio == nullptr) {

#ifndef __GK_DIFFUSER_H__
#define __GK_DIFFUSER_H__
...
typedef struct {
    ...
    const char* llm_adapter_path;  // Qwen3 -> T5-XXL bridge adapter (PixArt with --llm instead of --t5xxl)
...
typedef struct {
    ...
    int video_frames;
    int fps;
    float audio_duration;  // seconds of audio for audio-generation models (ACE-Step)
    const char* lyrics;    // optional lyrics for music models (MiniMax Music 3); NULL/empty = instrumental
    float vace_strength;
    sd_tiling_params_t vae_tiling_params;
    sd_cache_params_t cache;
    sd_hires_params_t hires;
...
// The compute devices the engine can see, in the order it numbers them.
//
// `name` is exactly what --backend and --split-mode accept ("cuda0", "cpu"),
// which is the reason this exists: that numbering is the engine's, and it is
// not safe to guess from anywhere else. CUDA orders its devices fastest-first
// by default, so the engine's cuda0 is routinely a different card from
// nvidia-smi's GPU 0 - assigning a module to the wrong one is silent, and
// looks like the split having no effect.
//
// A device that the build has no code for does not appear here at all; it is
// reported once, at discovery, on the engine's log.
typedef struct {
    const char* name;         // "cuda0", "cpu" - what --backend accepts
    const char* description;  // what the driver calls it
    bool is_gpu;              // discrete or integrated, as against the host
    size_t memory_free;
    size_t memory_total;
} sd_device_info_t;

// Fills `out` with up to `max` devices and returns how many the engine has.
// A return larger than `max` means the array was too small; `out` may be NULL
// with `max` 0 to ask for the count alone. The strings belong to the engine
// and stay valid for the life of the process.
SD_API int sd_list_devices(sd_device_info_t* out, int max);
SD_API bool sd_ctx_supports_image_generation(const sd_ctx_t* sd_ctx);
SD_API bool sd_ctx_supports_video_generation(const sd_ctx_t* sd_ctx);
// audio-only models (ACE-Step): drive them through generate_video, which
// returns no frames and hands the result back via audio_out
...
SD_API float sd_get_default_txt_cfg(const sd_ctx_t* sd_ctx);
SD_API void sd_get_default_resolution(const sd_ctx_t* sd_ctx, int* width, int* height);

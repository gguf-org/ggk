    ...
    for (auto it = params_storage_blocks_.begin(); it != params_storage_blocks_.end();) {
        ...
        if (!can_release) {
            can_release = std::all_of(block->states.begin(),
                                      block->states.end(),
                                      [target_states, evict_resident](TensorState* state) {
                                          if (state == nullptr) {
                                              return true;
                                          }
                                          if (target_states != nullptr &&
                                          ...
                                          // A Disk-resident tensor's home is the file and its
                                          // params buffer is only a cache, so dropping one is
                                          // free. A ParamBackend tensor is meant to stay put,
                                          // and dropping it costs a re-read the next time it
                                          // is prepared - which is why it happens only when
                                          // the caller has decided the memory is worth more
                                          // than the read.
...
size_t ModelManager::evict_params_backend_params(const std::vector<ggml_tensor*>& tensors) {
    if (tensors.empty()) {
        return 0;
    }
    std::vector<TensorState*> required_states;
    ...
    // Measured before the release rather than reported by it, because a block
    // is only freed when every tensor in it is in the target set - so what was
    // asked for and what actually goes back are not the same number, and the
    // caller wants the second one.
    size_t freed_before = 0;
    for (const auto& block : params_storage_blocks_) {
        if (block->buffer != nullptr) {
            freed_before += ggml_backend_buffer_get_size(block->buffer);
        }
    }

    std::unordered_set<TensorState*> target_states(required_states.begin(), required_states.end());
    release_params_storage_blocks(false, &target_states, /*evict_resident =*/true);

    size_t freed_after = 0;
    for (const auto& block : params_storage_blocks_) {
        if (block->buffer != nullptr) {
            freed_after += ggml_backend_buffer_get_size(block->buffer);
        }
    }

    return freed_before > freed_after ? freed_before - freed_after : 0;
}

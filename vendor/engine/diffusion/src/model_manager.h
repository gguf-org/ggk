...
public:
    ...
    // Give back the params memory of tensors that are finished with, including
    // the ParamBackend-resident ones that release_params_backend_params leaves
    // alone. They stay registered and reload from the model file the next time
    // they are prepared, so this is a memory/re-read trade and not a teardown.
    // Returns the bytes actually freed.
    size_t evict_params_backend_params(const std::vector<ggml_tensor*>& tensors);
};

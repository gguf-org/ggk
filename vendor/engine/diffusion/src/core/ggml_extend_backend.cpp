...
ggml_status sd_backend_graph_compute_with_eval_callback(ggml_backend_t backend,
    ...
    for (int j0 = 0; j0 < n_nodes; ++j0) {
        ...
        // The graph struct is opaque, so the slice [j0, j1] is evaluated by the
        // kernels rather than by building a view of it here. The call is
        // synchronous; the synchronize below it is what the async form needed.
        status = ggml_compat_graph_compute_range(backend, gf, j0, j1 + 1);
...
int sd_list_devices(sd_device_info_t* out, int max) {
    ggml_backend_load_all_once();

    const int count = (int)ggml_backend_dev_count();

    // Lower-cased, because that is the spelling --backend parses: the registry
    // says "CUDA0" and every assignment in the CLI and the GUI is written
    // "cuda0". Handing back the registry's spelling would produce a list whose
    // entries do not work when pasted back in. Held statically because the
    // caller is promised the pointers outlive the call; the device set does not
    // change once the registry is built, so filling it repeatedly is harmless.
    static std::vector<std::string> names;
    names.resize((size_t)std::max(count, 0));

    for (int i = 0; i < count && i < max; ++i) {
        ggml_backend_dev_t dev = ggml_backend_dev_get((size_t)i);

        names[(size_t)i] = lower_copy(ggml_backend_dev_name(dev));

        const enum ggml_backend_dev_type type = ggml_backend_dev_type(dev);

        out[i].name        = names[(size_t)i].c_str();
        out[i].description = ggml_backend_dev_description(dev);
        out[i].is_gpu      = type == GGML_BACKEND_DEVICE_TYPE_GPU ||
                             type == GGML_BACKEND_DEVICE_TYPE_IGPU;

        out[i].memory_free  = 0;
        out[i].memory_total = 0;
        ggml_backend_dev_memory(dev, &out[i].memory_free, &out[i].memory_total);
    }

...
namespace sd::ggml_graph_cut {
    ...
    static int graph_leaf_index(ggml_cgraph* gf, const ggml_tensor* tensor) {
        ...
        const int n_leafs = ggml_compat_graph_n_leafs(gf);
        for (int i = 0; i < n_leafs; ++i) {
            if (ggml_compat_graph_leaf(gf, i) == tensor) {
                return i;
            }
        }
        return -1;
    ...
    int leaf_count(ggml_cgraph* gf) {
        GGML_ASSERT(gf != nullptr);
        return ggml_compat_graph_n_leafs(gf);
    }

    ggml_tensor* leaf_tensor(ggml_cgraph* gf, int leaf_index) {
        GGML_ASSERT(gf != nullptr);
        if (leaf_index < 0 || leaf_index >= leaf_count(gf)) {
            return nullptr;
        }
        return ggml_compat_graph_leaf(gf, leaf_index);
    }
    ...
    bool plan_matches_graph(ggml_cgraph* gf, const Plan& plan) {
        GGML_ASSERT(gf != nullptr);
        if (ggml_graph_n_nodes(gf) != plan.n_nodes || leaf_count(gf) != plan.n_leafs) {
            return false;
        }
        for (const auto& input_shape_ref : plan.input_shapes) {
            ggml_tensor* leaf = leaf_tensor(gf, input_shape_ref.leaf_index);
    ...
    ggml_cgraph* build_segment_graph(ggml_cgraph* gf,
        ...
        for (const auto& input : segment.input_refs) {
            ...
            ggml_compat_graph_add_leaf(segment_graph, current_input);
        }

...
__STATIC_INLINE__ ggml_tensor* ggml_ext_attention_ext(ggml_context* ctx,
                                                      ...
                                                      float attn_softcap = 0.0f) {  // tanh logit softcapping (e.g. Gemma2), manual path only
    int64_t L_q;
    int64_t L_k;
    int64_t C;
    int64_t N;
    ...
    if (flash_attn && attn_softcap == 0.0f) {
    ...
    if (kqv == nullptr) {
        ...
        if (attn_softcap != 0.0f) {
            kq = ggml_scale_inplace(ctx, kq, 1.0f / attn_softcap);
            kq = ggml_tanh(ctx, kq);
            kq = ggml_scale_inplace(ctx, kq, attn_softcap);
        }
        if (mask) {
...
protected:
    ...
    // Set per graph by detect_cpu_fallback_ops(): this graph has a node the
    // runtime backend cannot run, so it goes through the scheduler even on a
    // single device. See that function for why.
    bool graph_needs_cpu_fallback_    = false;
    bool cpu_fallback_notice_logged_  = false;
    ...
    // Whether this graph runs through the scheduler rather than straight on
    // the runtime backend. Several runtime backends is one reason; a node the
    // single runtime backend declines is the other.
    bool use_sched_for_graph() const {
        return is_multi_device() || graph_needs_cpu_fallback_;
    }

    // Does this graph contain a node the runtime backend cannot run?
    //
    // With several runtime backends the scheduler is already in the path and
    // places such a node on the CPU without anyone asking. With one backend it
    // is not: execute_graph hands the graph straight to it, and a node it
    // declines fails the *whole graph* - GGML_STATUS_FAILED, no fallback, no
    // partial result. The caller then reports a failed sub-model and carries
    // on, so the visible symptom is a degraded output from a run that looks
    // like it succeeded. A missing conv_transpose_1d kernel is how this was
    // found: the audio VAE died and the engine wrote a silent video and
    // exited 0.
    //
    // So: scan first, and route the graph through the scheduler when the scan
    // finds anything. The cost when it finds nothing is one supports_op call
    // per node against a graph we are about to spend seconds computing.
    //
    // This is deliberately re-evaluated per graph rather than latched on the
    // runner. A runner's graphs are not all alike - a VAE decoder and its
    // encoder differ - and the fast path should be given back to the graphs
    // that can use it.
    void detect_cpu_fallback_ops(ggml_cgraph* gf) {
        graph_needs_cpu_fallback_ = false;

        if (gf == nullptr || is_multi_device() || sd_backend_is_cpu(runtime_backend)) {
            return;
        }
        ...
        for (int i = 0; i < n_nodes; ++i) {
            ggml_tensor* node = ggml_graph_node(gf, i);
            // Pure layout ops carry no kernel; a backend that returns false for
            // one of them is answering a different question than the one asked.
            if (node->op == GGML_OP_NONE || node->op == GGML_OP_VIEW || node->op == GGML_OP_RESHAPE ||
                node->op == GGML_OP_PERMUTE || node->op == GGML_OP_TRANSPOSE) {
                continue;
            }
        ...
        graph_needs_cpu_fallback_ = true;

        // The scheduler only gives a backend - and so a buffer - to tensors it
        // sees in the graph, which is why set_backend_tensor_data flags them as
        // inputs when it already knows a scheduler is coming. Here it did not
        // know: the graph did not exist yet when those calls were made. Setting
        // the flag now is equivalent, because nothing has been allocated yet
        // and the graph holds these very tensors.
        if (!backend_tensor_data_map.empty()) {
            ...
            for (auto& kv : backend_tensor_data_map) {
                if (kv.first != nullptr && graph_tensor_set.find(kv.first) != graph_tensor_set.end()) {
                    ggml_set_input(kv.first);
                }
            }
        }

        if (!cpu_fallback_notice_logged_) {
            const char* op_name = first_node != nullptr ? ggml_op_name(first_node->op) : "?";
            LOG_WARN("%s: %d of %d nodes are not supported by %s (first: %s); running those on the CPU. "
                     "This is correct but slow - a CUDA kernel for %s would avoid the round trip.",
                     get_desc().c_str(),
                     n_unsupported,
                     n_nodes,
                     ...
                     op_name,
                     op_name);
            cpu_fallback_notice_logged_ = true;
        }
    }

    bool alloc_compute_buffer(ggml_cgraph* gf) {
        if (use_sched_for_graph()) {
    ...
    std::optional<sd::Tensor<T>> execute_graph(ggml_cgraph* gf,
        ...
        // Before anything is allocated: this decides whether the graph is
        // allocated and computed by the scheduler or by the runtime backend
        // alone, and both of the calls below branch on the answer.
        detect_cpu_fallback_ops(gf);
        ...
        if (use_sched_for_graph()) {
            ...
            // reset clears the pins; re-apply before alloc. On the single-device
            // fallback path this is what keeps the graph on the GPU: it pins
            // every node the backend does support, leaving only the ones it
            // declines for the scheduler to place on the CPU.
            pin_multi_device_nodes(gf);
        ...
        if (use_sched_for_graph()) {
            if (sd_get_backend_eval_callback() != nullptr && !multi_device_eval_callback_warned) {
                LOG_WARN("%s: eval callback is not supported when the graph runs through the scheduler "
                         "(%s); ignoring",
                         get_desc().c_str(),
                         is_multi_device() ? "multiple runtime backends" : "CPU fallback for unsupported ops");

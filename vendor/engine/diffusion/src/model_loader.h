...
public:
    ...
    bool load_float_tensor(const std::string& name,
                           ...
                           int n_threads = 0,
                           bool use_mmap = false);
    // Reads a tensor's raw on-disk bytes without any type conversion; used for
    // non-weight payloads like tokenizer.json blobs embedded in model files.
    bool read_tensor_bytes(const std::string& name, std::string& out);

...
SDVersion ModelLoader::get_sd_version() {
    ...
    for (auto& [name, tensor_storage] : tensor_storage_map) {
        ...
        if (tensor_storage.name.find("model.diffusion_model.adaln_t_table") != std::string::npos &&
            tensor_storage_map.find("model.diffusion_model.audio_patch_proj.weight") != tensor_storage_map.end() &&
            tensor_storage_map.find("model.diffusion_model.video_patch_proj.weight") != tensor_storage_map.end()) {
            return VERSION_MINIMAX_H3;
        }
        if (tensor_storage.name.find("model.diffusion_model.genre_embedder.weight") != std::string::npos &&
            tensor_storage_map.find("model.diffusion_model.lyric_embs.weight") != tensor_storage_map.end()) {
            return VERSION_ACE_STEP;
        }
        if (tensor_storage.name.find("model.diffusion_model.diffusion_transformer.preprocess_conv.weight") != std::string::npos &&
            tensor_storage_map.find("model.diffusion_model.cond_layer_logits") != tensor_storage_map.end()) {
            return VERSION_MINIMAX_MUSIC3;
        }
        ...
        if (tensor_storage.name.find("model.diffusion_model.cap_embedder.0.weight") != std::string::npos) {
            // Lumina2 and Z-Image share the NextDiT naming; Lumina2's per-layer adaLN is
            // nn.Sequential(SiLU, Linear) (index 1), Z-Image's is a bare Linear (index 0).
            if (tensor_storage_map.find("model.diffusion_model.layers.0.adaLN_modulation.1.weight") != tensor_storage_map.end()) {
                return VERSION_LUMINA2;
            }
        ...
        if (tensor_storage.name.find("model.diffusion_model.adaln_single.emb.timestep_embedder.linear_1.bias") != std::string::npos) {
            // PixArt shares LTXV's adaLN-single naming; only PixArt patchifies with a
            // conv (LTXV uses a linear patchify_proj).

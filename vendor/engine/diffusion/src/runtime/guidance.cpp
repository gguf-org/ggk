...
namespace sd::guidance {
    ...
    GuiderOutput ClassifierFreeGuidance::forward(const GuidanceInput& input,
        ...
        if (renorm_ > 0.0f && (has_tensor(input.pred_uncond) || has_tensor(input.pred_img_uncond))) {
            const float* cond = pred_cond.data();
            const float* pred = output.pred.data();
            double cond_sq    = 0.0;
            double pred_sq    = 0.0;
            int64_t n         = pred_cond.numel();
            for (int64_t i = 0; i < n; i++) {
                cond_sq += (double)cond[i] * cond[i];
                pred_sq += (double)pred[i] * pred[i];
            }
            float max_norm  = (float)std::sqrt(cond_sq) * renorm_;
            float pred_norm = (float)std::sqrt(pred_sq);
            if (pred_norm > max_norm && pred_norm > 0.0f) {
                output.pred = output.pred * (max_norm / pred_norm);
            }
        }

...
private:
    ...
    bool had_high_dimensional_tensors_ = false;
    ...
    bool skip_metadata_value(std::ifstream& fin, uint32_t type) {
        switch (static_cast<GGUFMetadataType>(type)) {
            ...
            case GGUFMetadataType::ARRAY: {
                ...
                for (uint64_t i = 0; i < len; i++) {
                    if (!skip_metadata_value(fin, elem_type))
                        return false;
                }
                return true;
            }
    ...
    bool read_metadata(std::ifstream& fin) {
        ...
        if (key == "general.alignment" && static_cast<GGUFMetadataType>(type) == GGUFMetadataType::UINT32) {
        ...
        return skip_metadata_value(fin, type);
    }
    ...
    GGUFTensorInfo read_tensor_info(std::ifstream& fin) {
        ...
        if (n_dims > GGML_MAX_DIMS) {
            had_high_dimensional_tensors_ = true;
...
public:
    bool load(const std::string& file_path) {
        tensors_.clear();
        alignment_                       = 32;
        had_high_dimensional_tensors_    = false;
    ...
    bool had_high_dimensional_tensors() const { return had_high_dimensional_tensors_; }
};

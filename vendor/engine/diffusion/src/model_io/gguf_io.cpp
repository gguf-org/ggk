...
bool read_gguf_file(const std::string& file_path,
    ...
    const bool custom_reader_loaded = gguf_reader.load(file_path);

    auto load_from_custom_reader = [&]() {
        const size_t data_offset = gguf_reader.data_offset();
        for (const auto& gguf_tensor_info : gguf_reader.tensors()) {
            tensor_storages.emplace_back(
    ...
    // Standard GGUF is limited to GGML_MAX_DIMS. Some video VAEs store
    // convolution weights with five dimensions, which the extended reader
    // safely flattens into the final ggml dimension.
    if (custom_reader_loaded && gguf_reader.had_high_dimensional_tensors()) {
        load_from_custom_reader();
        return true;
    }

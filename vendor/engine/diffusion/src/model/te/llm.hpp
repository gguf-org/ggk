...
namespace LLM {
    ...
    struct LLMConfig {
        ...
        float attn_logit_softcapping    = 0.f;  // Gemma2: tanh(logits/cap)*cap before softmax
        ...
        // trainer5/: a QWEN3_VL-versioned model whose --llm is actually a small
        // plain-Qwen3 student (bridge adapter present): rope the TEXT stack like
        // QWEN3 (1-D positions, theta 1e6 -- the student's native pretraining)
        // while keeping the Qwen3-VL vision tower for the mmproj.
        bool text_rope_qwen3 = false;
        // Gemma2 HF checkpoints name the extra norms pre/post_feedforward_layernorm;
        // llama.cpp-converted ggufs land on the Gemma3-style names after the generic
        // llm name map (post_attention_norm / post_attention_layernorm / post_ffw_norm).
        bool gemma2_hf_norm_names = false;
        ...
        static LLMConfig detect_from_weights(const String2TensorStorage& tensor_storage_map,
            ...
            if (arch == LLMArch::MISTRAL_SMALL_3_2 || arch == LLMArch::MINISTRAL_3_3B) {
                ...
                config.normalize_input         = true;
                config.attn_logit_softcapping  = 50.f;
            ...
            for (const auto& [name, tensor_storage] : tensor_storage_map) {
                ...
                if (contains(name, "model.norm.weight")) {
                    config.have_final_norm = true;
                }
                if (contains(name, "layers.0.pre_feedforward_layernorm.weight")) {
                    config.gemma2_hf_norm_names = true;
                }
    ...
    public:
        Attention(const LLMConfig& config)
            : arch(config.arch),
              ...
              text_rope_qwen3(config.text_rope_qwen3) {
        ...
        ggml_tensor* forward(GGMLRunnerContext* ctx,
            ...
            } else if (arch == LLMArch::QWEN3 || (arch == LLMArch::QWEN3_VL && text_rope_qwen3)) {
            ...
            if (arch == LLMArch::GPT_OSS_20B) {
                ...
                x = ggml_ext_attention_ext(ctx->ggml_ctx, ctx->backend, q, k, v, num_heads, attention_mask, true, false, 1.0f, attn_softcap);  // [N, n_token, hidden_size]
            }
    ...
    public:
        TransformerBlock(const LLMConfig& config, int layer_index)
            ...
            if (config.arch == LLMArch::GEMMA3_12B) {
                ...
                if (config.gemma2_hf_norm_names) {
                    ...
                    // llama.cpp gguf converted through the generic llm name map:
                    // attn_post_norm -> post_attention_norm, ffn_norm (= HF
    ...
    public:
        ...
        ggml_tensor* forward_embeds(GGMLRunnerContext* ctx,
            ...
            auto norm = blocks.count("norm") != 0
                            ? std::dynamic_pointer_cast<LLMRMSNorm>(blocks["norm"])
                            : nullptr;
            ...
            auto normed_x = norm != nullptr ? norm->forward(ctx, x) : x;
            if (return_all_hidden_states) {
    ...
    public:
        LLMRunner(LLMArch arch,
            ...
            config.text_rope_qwen3 = text_rope_qwen3 && arch == LLMArch::QWEN3_VL;

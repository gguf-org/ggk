...
#include "model/diffusion/ace_step.hpp"
#include "model/diffusion/ace_step_lyric.hpp"
...
void calculate_alphas_cumprod(float* alphas_cumprod,
    ...
    for (int i = 0; i < timesteps; i++) {
        float beta;
        if (scaled_linear) {
            beta = powf(ls_sqrt + amount * ((float)i / (timesteps - 1)), 2.0f);
        } else {
            // plain linear betas (ADM/DDPM), what PixArt and the original DiT train on
            beta = linear_start + (linear_end - linear_start) * ((float)i / (timesteps - 1));
        }
        product *= 1.0f - beta;
...
public:
    ...
    // Kept so that the decision to evict the text encoder after conditioning
    // has something to weigh. Both are params sizes as registered, in bytes.
    size_t text_encoder_params_size = 0;
    size_t diffusion_params_size    = 0;
    size_t vae_params_size          = 0;
    ...
    // Give the text encoder's weights back after conditioning.
    //
    // Conditioning happens once, before the first denoising step, and the text
    // encoder is dead weight from then until the generation ends. On a card
    // where it and the diffusion model both fit that costs nothing and this
    // does nothing. Where they do not both fit, the driver starts paging and
    // the denoiser reads its weights across the bus every step: measured on a
    // 6 GB card with a 4.3 GB text encoder and a 2.2 GB DiT, a MageFlow step
    // went from 16.19 s to 1.99 s once the encoder was out of the way.
    //
    // The weights stay registered and reload from the model file if anything
    // asks for them again, so the trade is a re-read against the paging, and
    // the test below is what decides which one is cheaper.
    void evict_text_encoder_params() {
        if (cond_stage_model == nullptr || model_manager == nullptr) {
            return;
        }

        // The escape hatch. The eviction is a bet that conditioning is over,
        // and it is the caller of an embedded engine - not this file - who
        // knows about a use that would make that bet wrong.
        const char* keep_resident = getenv("SD_KEEP_TE_RESIDENT");
        if (keep_resident != nullptr && keep_resident[0] != '0') {
            LOG_DEBUG("keeping text encoder params resident (SD_KEEP_TE_RESIDENT)");
            return;
        }

        ggml_backend_t te_params_backend        = backend_manager.params_backend(SDBackendModule::TE);
        ggml_backend_t diffusion_params_backend = backend_manager.params_backend(SDBackendModule::DIFFUSION);
        if (te_params_backend == nullptr || diffusion_params_backend == nullptr) {
            return;
        }

        // Nothing to win: freeing host memory does not give the denoiser room
        // on the device, and freeing memory on a device the denoiser does not
        // use gives it room it cannot reach.
        if (sd_backend_is_cpu(te_params_backend)) {
            return;
        }
        ggml_backend_dev_t te_dev        = ggml_backend_get_device(te_params_backend);
        ggml_backend_dev_t diffusion_dev = ggml_backend_get_device(diffusion_params_backend);
        if (te_dev == nullptr || te_dev != diffusion_dev) {
            return;
        }

        // Do the text encoder and the denoiser fit on this device at the same
        // time?
        //
        // Asked against the device's total memory and the params sizes we
        // registered, deliberately not against the driver's free-memory figure.
        // Once an allocation has been demoted to host memory that figure counts
        // it as free, so a card that is thrashing reports room to spare - on the
        // 6 GB card that motivated this it said 744 MB free while the denoiser
        // was reading its weights across the bus at 20 s/step. Total memory and
        // what we allocated are both facts; free memory under pressure is not.
        size_t free_bytes = 0, total_bytes = 0;
        ggml_backend_dev_memory(diffusion_dev, &free_bytes, &total_bytes);
        if (total_bytes == 0) {
            return;
        }

        // Room the params do not get: compute buffers, whatever else has the
        // device open, and the driver's own overhead. A tenth of the card is a
        // rough number for a rough purpose - the decision only turns on it in a
        // narrow band, and everywhere else the params either obviously fit or
        // obviously do not.
        const size_t reserve  = std::max<size_t>(total_bytes / 10, 256ull * 1024 * 1024);
        const size_t usable   = total_bytes > reserve ? total_bytes - reserve : 0;
        const size_t resident = text_encoder_params_size + diffusion_params_size + vae_params_size;

        if (resident <= usable) {
            LOG_DEBUG("keeping text encoder params resident (%.2f MB of params, %.2f MB usable on %s)",
                      resident / 1024.0 / 1024.0,
                      usable / 1024.0 / 1024.0,
                      ggml_backend_dev_name(diffusion_dev));
            return;
        }

        std::map<std::string, ggml_tensor*> tensor_map;
        cond_stage_model->get_param_tensors(tensor_map);
        std::vector<ggml_tensor*> tensors;
        tensors.reserve(tensor_map.size());
        for (auto& kv : tensor_map) {
            if (kv.second != nullptr) {
                tensors.push_back(kv.second);
            }
        }

        const size_t freed = model_manager->evict_params_backend_params(tensors);
        if (freed > 0) {
            LOG_INFO("evicted text encoder params after conditioning (%.2f MB freed; "
                     "%.2f MB of params would not fit in %.2f MB usable on %s)",
                     freed / 1024.0 / 1024.0,
                     resident / 1024.0 / 1024.0,
                     usable / 1024.0 / 1024.0,
                     ggml_backend_dev_name(diffusion_dev));
        } else {
            LOG_DEBUG("text encoder params could not be evicted (still in use)");
        }
    }
    ...
    bool register_row_split_runner_params(const std::string& desc,
        ...
        for (const auto& kv : group_tensors) {
            if (sd::layer_split_tensor_block_index(kv.first) >= 0 &&
    ...
    void refresh_compvis_denoiser_sigmas() {
        ...
        if (file_alphas_cumprod.size() == TIMESTEPS) {
            ...
            calculate_alphas_cumprod(alphas_cumprod.data(), 0.0001f, 0.02f, TIMESTEPS, false);
        } else {
    ...
    bool init(const sd_ctx_params_t* sd_ctx_params) {
        ...
        if (strlen(SAFE_STR(sd_ctx_params->llm_path)) > 0) {
            ...
            // ACE-Step conditions on UMT5, so a text encoder passed via --llm
            // routes to the T5 slot (and through the T5 name conversion).
            std::string prefix = sd_version_is_ace_step(model_loader.get_sd_version())
                                     ? "text_encoders.t5xxl.transformer."
                                     : "text_encoders.llm.";
            if (!model_loader.init_from_file(sd_ctx_params->llm_path, prefix)) {
        ...
        if (strlen(SAFE_STR(sd_ctx_params->llm_adapter_path)) > 0) {
            ...
            if (!model_loader.init_from_file(sd_ctx_params->llm_adapter_path, "text_encoders.llm_adapter.")) {
                LOG_WARN("loading llm adapter from '%s' failed", sd_ctx_params->llm_adapter_path);
            }
        }
        ...
        if (tensor_storage_map.find("text_encoders.llm.tokenizer_json") != tensor_storage_map.end()) {
            ...
            if (sd_version_is_lens(version)) {
                ...
                vocab_family = "qwen2";
            } else if (sd_version_is_minimax_music3(version)) {
                vocab_family = "minimax_music3";
            }
        ...
        if (sd_ctx_params->lora_apply_mode == LORA_APPLY_AUTO) {
            ...
            if (row_split_active()) {
                LOG_WARN("row-split tensors do not support the immediately LoRA apply mode; LoRAs will not be applied to them");
            }
            apply_lora_immediately = true;
        } else {
            apply_lora_immediately = false;
...

        {
            ...
            if (sd_version_is_sd3(version)) {
                ...
                cond_stage_model = std::make_shared<LLMEmbedder>(backend_for(SDBackendModule::TE),
                                                                 ...
                                                                 false,
                                                                 model_manager);
                diffusion_model  = std::make_shared<Lumina2::Lumina2Runner>(backend_for(SDBackendModule::DIFFUSION),
                                                                           tensor_storage_map,
                                                                           "model.diffusion_model",
                                                                           version,
                                                                           model_manager);
                ...
                auto y_embedding      = tensor_storage_map.find("model.diffusion_model.caption_projection.y_embedding");
                if (y_embedding != tensor_storage_map.end() && y_embedding->second.n_dims == 2) {
                    caption_length = static_cast<size_t>(y_embedding->second.ne[1]);
                }
                cond_stage_model = std::make_shared<PixArtT5Embedder>(backend_for(SDBackendModule::TE),
                                                                      tensor_storage_map,
                                                                      caption_length,
                                                                      model_manager);
                diffusion_model  = std::make_shared<PixArt::PixArtRunner>(backend_for(SDBackendModule::DIFFUSION),
                                                                         tensor_storage_map,
                                                                         "model.diffusion_model",
                                                                         model_manager);
                ...
                diffusion_model  = std::make_shared<Lens::LensRunner>(backend_for(SDBackendModule::DIFFUSION),
                                                                     ...
                                                                     "model.diffusion_model",
                                                                     model_manager);
            } else if (sd_version_is_ace_step(version)) {
                cond_stage_model = std::make_shared<ACEStepConditioner>(backend_for(SDBackendModule::TE),
                                                                        tensor_storage_map,
                                                                        model_manager);
                diffusion_model  = std::make_shared<AceStep::AceStepRunner>(backend_for(SDBackendModule::DIFFUSION),
                                                                           tensor_storage_map,
                                                                           "model.diffusion_model",
                                                                           model_manager);
            } else if (sd_version_is_minimax_music3(version)) {
                cond_stage_model = std::make_shared<MiniMaxMusic3Conditioner>(backend_for(SDBackendModule::TE),
                                                                              tensor_storage_map,
                                                                              model_manager);
                diffusion_model  = std::make_shared<MiniMaxMusic3::Music3FlowRunner>(backend_for(SDBackendModule::DIFFUSION),
                                                                                    tensor_storage_map,
                                                                                    "model.diffusion_model",
                                                                                    model_manager);
            ...
            auto create_vae = [&]() -> std::shared_ptr<VAE> {
                if (sd_version_is_ltxav(version)) {
                    return std::make_shared<LTXVideoVAE>(backend_for(SDBackendModule::VAE),
                                                         ...
                                                         version,
                                                         model_manager);
                } else if (sd_version_uses_minimax_h3_vae(version)) {
            ...
            if (version == VERSION_CHROMA_RADIANCE || version == VERSION_HIDREAM_O1 || sd_version_is_minit2i(version) ||
                sd_version_is_ace_step(version) || sd_version_is_minimax_music3(version)) {
                LOG_INFO(sd_version_is_ace_step(version) || sd_version_is_minimax_music3(version)
                             ? "using FakeVAE (latents decode through the music vocoder)"
                             : "using FakeVAE");
            ...
            if (sd_version_is_minimax_music3(version)) {
                // the vocoder arrives through --vae; the loader renames that
                // prefix to "first_stage_model."
                bool has_music3_vae = tensor_storage_map.find("first_stage_model.dec_in_proj.weight") != tensor_storage_map.end();
                if (!has_music3_vae) {
                    LOG_ERROR("MiniMax Music 3 needs its vocoder; pass it with --vae");
                    return false;
                }
                audio_vae_model = std::make_shared<MiniMaxMusic3::Music3VocoderRunner>(backend_for(SDBackendModule::VAE),
                                                                                       tensor_storage_map,
                                                                                       "first_stage_model",
                                                                                       model_manager);
                if (!register_runner_params("MiniMax Music 3 vocoder",
                                            audio_vae_model,
                                            SDBackendModule::VAE,
                                            &vae_params_mem_size)) {
                    return false;
                ...
                // the DCAE + vocoder arrive through --vae; the loader renames
                // that prefix to "first_stage_model."
                bool has_ace_vae = tensor_storage_map.find("first_stage_model.dcae.decoder.conv_in.weight") != tensor_storage_map.end();
                if (!has_ace_vae) {
                    LOG_ERROR("ACE-Step needs its music VAE (dcae + vocoder); pass it with --vae");
                    return false;
                }
                audio_vae_model = std::make_shared<AceStep::AceVAERunner>(backend_for(SDBackendModule::VAE),
                                                                          tensor_storage_map,
                                                                          "first_stage_model",
                                                                          model_manager);
                if (!register_runner_params("ACE music VAE",
                                            audio_vae_model,
                                            SDBackendModule::VAE,
                                            &vae_params_mem_size)) {
                    return false;
                ...
                if (!register_runner_params(sd_version_is_minimax_h3(version) ? "MiniMax H3 audio VAE" : "LTX audio VAE",
                                            audio_vae_model,
                                            SDBackendModule::VAE,
                                            &vae_params_mem_size)) {
                    return false;
        ...
        if (sd_version_is_pixart(version)) {
            // the null-caption buffer is training-time dropout only
            ignore_tensors.insert("model.diffusion_model.caption_projection.y_embedding");
        }
        ...
        if (sd_version_is_minimax_h3(version)) {
            // T2V only needs the text tower; reference-video vision support
            // can be enabled independently in a later integration.
            ignore_tensors.insert("text_encoders.llm.visual.");
        }
        if (sd_version_is_minimax_music3(version)) {
            ignore_tensors.insert("text_encoders.llm.tokenizer_json");
...

        {
            ...
            text_encoder_params_size = text_encoder_params_mem_size;
            diffusion_params_size    = unet_params_mem_size;
            vae_params_size          = vae_params_mem_size;
                ...
                if (sd_version_is_sd2(version)) {
                    ...
                    if (tensor_storage_map.find("edm_vpred.sigma_max") != tensor_storage_map.end()) {
                        ...
                        pred_type = EPS_PRED;
                           ...
                           sd_version_is_lumina2(version) ||
                           sd_version_is_ace_step(version) ||
                           sd_version_is_minimax_music3(version)) {
                    pred_type = FLOW_PRED;
    ...
    std::vector<float> prepare_sample_timesteps(float sigma,
        ...
        if (sd_version_is_lumina2(version)) {
            // NextDiT wants 1 - sigma in [0, 1]
            return std::vector<float>{(1000.f - t) / 1000.f};
        }
    ...
    sd::Tensor<float> sample(const std::shared_ptr<DiffusionModelRunner>& work_diffusion_model,
        ...
        // Lumina2 applies CFG renormalization by default (renorm_cfg = 1 in the
        // reference implementation): the guided prediction's norm is capped at
        // the cond prediction's norm, which prevents high-CFG washout/burn.
        float cfg_renorm = sd_version_is_lumina2(version) ? 1.0f : 0.0f;
        sd::guidance::ClassifierFreeGuidance classifier_free_guidance(cfg_scale, img_cfg_scale, cfg_renorm);
    ...
    int get_diffusion_model_down_factor() {
        ...
        if (sd_version_is_pixart(version)) {
            // PixArt is a DiT over the SD VAE: only the 2x2 patch grid constrains the size
            return 2;
        }
    ...
    int video_frames_to_latent_frames(int frames) {
        ...
        if (sd_version_is_ltxav(version)) {
            ...
            latent_frames = frames <= 5 ? 2 : ((frames - 5 + 16) / 17) * 5 + 2;
    ...
    float get_flow_shift() const {
        auto flow_denoiser = std::dynamic_pointer_cast<DiscreteFlowDenoiser>(denoiser);
        return flow_denoiser != nullptr ? flow_denoiser->shift : 1.f;
    }
...
void sd_vid_gen_params_init(sd_vid_gen_params_t* sd_vid_gen_params) {
    *sd_vid_gen_params = {};
    ...
    sd_vid_gen_params->fps                                   = 24;
    sd_vid_gen_params->audio_duration                        = 60.f;
...
static bool sd_version_supports_video_generation(SDVersion version) {
    return version == VERSION_SVD ||
           sd_version_is_wan(version) ||
           sd_version_is_ltxav(version) ||
           sd_version_is_minimax_h3(version);
}

static bool sd_version_supports_audio_generation(SDVersion version) {
    return sd_version_is_ace_step(version) || sd_version_is_minimax_music3(version);
}

static bool sd_version_supports_image_generation(SDVersion version) {
    return !sd_version_supports_video_generation(version) &&
           !sd_version_supports_audio_generation(version);
}
...
SD_API bool sd_ctx_supports_audio_generation(const sd_ctx_t* sd_ctx) {
    if (sd_ctx == nullptr || sd_ctx->sd == nullptr) {
        return false;
    }
    return sd_version_supports_audio_generation(sd_ctx->sd->version);
}
...
enum sample_method_t sd_get_default_sample_method(const sd_ctx_t* sd_ctx) {
    if (sd_ctx != nullptr && sd_ctx->sd != nullptr) {
        ...
        if (sd_version_is_pixart(sd_ctx->sd->version)) {
            // PixArt's reference pipeline samples deterministically (DPM-Solver++)
            return EULER_SAMPLE_METHOD;
        }
        if (sd_version_is_ace_step(sd_ctx->sd->version)) {
            // FlowMatchEulerDiscreteScheduler in the reference pipeline
            return EULER_SAMPLE_METHOD;
        }
...
float sd_get_default_txt_cfg(const sd_ctx_t* sd_ctx) {
    if (sd_ctx != nullptr && sd_ctx->sd != nullptr) {
        if (sd_version_is_lumina2(sd_ctx->sd->version)) {
            return 4.0f;
        }
        if (sd_version_is_pixart(sd_ctx->sd->version)) {
            return 4.5f;
        ...
        if (sd_version_is_ace_step(sd_ctx->sd->version)) {
            // the reference uses APG at 15 over half the schedule; with plain
            // CFG across every step ~5 is the equivalent working point
            return 5.0f;
        }
        if (sd_version_is_minimax_music3(sd_ctx->sd->version)) {
            // the reference flow guidance_scale
            return 1.7f;
        }
    }
    return 7.0f;
...
void sd_get_default_resolution(const sd_ctx_t* sd_ctx, int* width, int* height) {
    int w = 512;
    int h = 512;
    if (sd_ctx != nullptr && sd_ctx->sd != nullptr) {
        if (sd_version_is_lumina2(sd_ctx->sd->version)) {
            // Lumina Image 2.0 is 1024-native; 512 and below collapses to textures.
            w = 1024;
            h = 1024;
        }
        if (sd_version_is_pixart(sd_ctx->sd->version)) {
            // The position grid is only interpolated correctly around the size the
            // checkpoint was trained at, which the runner already worked out.
            auto pixart = std::dynamic_pointer_cast<PixArt::PixArtRunner>(sd_ctx->sd->diffusion_model);
            if (pixart) {
                int native = (int)pixart->config.sample_size * sd_ctx->sd->get_vae_scale_factor();
                if (native > 0) {
                    w = native;
                    h = native;
                }
...
struct GenerationRequest {
    ...
    GenerationRequest(sd_ctx_t* sd_ctx, const sd_vid_gen_params_t* sd_vid_gen_params) {
        ...
        if (sd_version_is_minimax_h3(sd_ctx->sd->version) && fps != 24) {
            LOG_WARN("MiniMax H3 uses a fixed 24 fps timeline; overriding requested fps %d", fps);
            fps = 24;
        }
        vae_scale_factor            = sd_ctx->sd->get_vae_scale_factor();
        diffusion_model_down_factor = sd_ctx->sd->get_diffusion_model_down_factor();
...
static sd::Tensor<float> unpack_minimax_h3_audio_latent(const sd::Tensor<float>& packed_latent,
    ...
    int64_t required_values = static_cast<int64_t>(audio_length) * 2 * 32;
    int64_t packed_values   = (packed_latent.shape()[3] - video_channels) * spatial_size;
    if (packed_latent.shape()[3] <= video_channels || packed_values < required_values) {
        return {};
    }
    sd::Tensor<float> audio_latent({audio_length, 2, 32, 1});
    const float* audio_src = packed_latent.data() +
...
static int get_minimax_h3_num_audio_latents(int frames) {
    constexpr float kVideoFps       = 24.f;
    constexpr float kAudioLatentFps = 40.f;
    return std::max(1, static_cast<int>(std::lround(frames * kAudioLatentFps / kVideoFps)));
}
...
static std::optional<ImageGenerationLatents> prepare_image_generation_latents(sd_ctx_t* sd_ctx,
    ...
    for (size_t i = 0; i < ref_images.size(); i++) {
        ...
        if (ref_image_params.resize_before_vae && !sd_version_is_pid(sd_ctx->sd->version)) {
            ...
            vae_height = std::max(static_cast<double>(factor), round(vae_height / factor) * factor);
            vae_width  = std::max(static_cast<double>(factor), round(vae_width / factor) * factor);
...
static std::optional<ImageGenerationEmbeds> prepare_image_generation_embeds(sd_ctx_t* sd_ctx,
    ...
    condition_params.ref_images            = ref_image_params.pass_to_vlm ? &latents->ref_images : nullptr;
...
SD_API bool generate_image(sd_ctx_t* sd_ctx,
    ...
    RefImageParams ref_image_params = sd_ctx->sd->resolve_ref_image_params(nullptr);
    if (sd_img_gen_params->increase_ref_index) {
        ref_image_params.ref_index_mode = Rope::RefIndexMode::INCREASE;
    }
    if (sd_img_gen_params->auto_resize_ref_image) {
        ref_image_params.resize_before_vae = true;
    }
    ...
    // After the call above has returned, so that the conditioner's own
    // runner_done has already dropped its compute buffer and unstaged its
    // tensors - params blocks are only released once nothing holds them.
    sd_ctx->sd->evict_text_encoder_params();
...
static std::optional<ImageGenerationLatents> prepare_video_generation_latents(sd_ctx_t* sd_ctx,
    ...
    } else if (sd_version_is_minimax_h3(sd_ctx->sd->version)) {
...
// ACE-Step text2music: denoise a [8, 16, T] mel-latent with the reference
// flow-matching schedule (shift 3.0), then decode it through the music VAE.
// The unconditional branch is the reference null condition - zeros in place
// of the UMT5 text hidden states (a negative prompt has no effect).
static bool generate_ace_audio(sd_ctx_t* sd_ctx,
                               const sd_vid_gen_params_t* sd_vid_gen_params,
    ...
    float duration = sd_vid_gen_params->audio_duration;
    if (duration <= 0.f) {
        duration = 60.f;
    }
    // one latent frame covers 8 mel hops of 512 samples at 44.1 kHz
    int frame_length = (int)(duration * 44100.0 / 512.0 / 8.0);
    frame_length     = std::max(8, std::min(frame_length, (int)(240.0 * 44100.0 / 512.0 / 8.0)));
    LOG_INFO("generate_audio: %.1fs -> %d latent frames, %zu steps",
             duration,
             frame_length,
             plan.sigmas.size() > 0 ? plan.sigmas.size() - 1 : 0);

    ConditionerRunnerDoneOnExit conditioner_runner_done{sd_ctx->sd->cond_stage_model.get()};

    ConditionerParams condition_params;
    condition_params.text = request.prompt;
    SDCondition cond      = sd_ctx->sd->cond_stage_model->get_learned_condition(sd_ctx->sd->n_threads,
                                                                                condition_params);
    if (cond.c_crossattn.empty()) {
        LOG_ERROR("ACE-Step conditioning failed (is the UMT5 text encoder loaded?)");
        return false;
    }
    ...
    if (request.use_uncond) {
        if (!request.negative_prompt.empty()) {
            LOG_WARN("ACE-Step's null condition is fixed (zeros); the negative prompt is ignored");
        }
        uncond.c_crossattn = sd::Tensor<float>::zeros(cond.c_crossattn.shape());
    }

    sd_ctx->sd->evict_text_encoder_params();

    // Lyrics: tokenize with the voice-BPE lyric tokenizer, run the conformer
    // lyric encoder once, and append the hidden states to the conditional
    // branch's cross-attention sequence. The reference zeroes the lyric mask
    // on the unconditional branch, so uncond stays lyric-free.
    std::string lyrics;
    if (sd_vid_gen_params->lyrics != nullptr) {
        lyrics = sd_vid_gen_params->lyrics;
    }
    if (!lyrics.empty()) {
        auto ace = std::dynamic_pointer_cast<AceStep::AceStepRunner>(sd_ctx->sd->diffusion_model);
        if (ace == nullptr || !ace->config.has_lyric_branch) {
            LOG_WARN("this ACE-Step checkpoint has no lyric encoder; --lyrics is ignored");
        } else {
            static const AceStep::AceLyricTokenizer lyric_tokenizer;
            if (!lyric_tokenizer.valid) {
                LOG_ERROR("the embedded ACE-Step lyric vocab failed to parse");
                return false;
            }
            std::vector<int32_t> lyric_ids = lyric_tokenizer.tokenize_lyrics(lyrics);
            LOG_INFO("ACE-Step lyrics: %zu tokens", lyric_ids.size());
            sd::Tensor<float> lyric_hidden = ace->encode_lyrics(sd_ctx->sd->n_threads, lyric_ids);
            if (lyric_hidden.empty()) {
                LOG_ERROR("ACE-Step lyric encoding failed");
                return false;
            }
            cond.c_vector = std::move(lyric_hidden);
        }
    }

    sd::Tensor<float> init_latent = sd::Tensor<float>::zeros({frame_length, 16, 8, 1});
    sd::Tensor<float> noise       = sd::Tensor<float>::randn_like(init_latent, sd_ctx->sd->rng);
    const RefImageParams empty_ref_image_params;
    ...
    sd::Tensor<float> final_latent = sd_ctx->sd->sample(sd_ctx->sd->diffusion_model,
                                                        ...
                                                        std::move(noise),
                                                        cond,
                                                        request.use_uncond ? uncond : SDCondition(),
                                                        SDCondition(),
                                                        sd::Tensor<float>(),
                                                        0.f,
...

    {
        ...
        for (int64_t i = 0; i < final_latent.numel(); ++i) {
            sum += d[i];
            sum2 += (double)d[i] * d[i];
            amax = std::max(amax, (double)std::fabs(d[i]));
        }
        double mean = sum / final_latent.numel();
        LOG_DEBUG("ace latent stats: mean=%.4f std=%.4f absmax=%.4f",
                  mean, std::sqrt(sum2 / final_latent.numel() - mean * mean), amax);
    }
    ...
    if (sd_ctx->sd->audio_vae_model == nullptr) {
        LOG_ERROR("no music VAE loaded; cannot decode the audio latent");
        return false;
    }
    auto waveform = sd_ctx->sd->audio_vae_model->decode(sd_ctx->sd->n_threads, final_latent);
    if (waveform.empty()) {
        LOG_ERROR("audio latent decode failed");
        return false;
    }

    if (audio_out != nullptr) {
        *audio_out = waveform_to_sd_audio(sd_ctx->sd, waveform);
        if (*audio_out == nullptr) {
            LOG_ERROR("failed to package the generated audio");
            return false;
        }
    }
    ...
    sd_ctx->sd->lora_stat();
    int64_t t1 = ggml_time_ms();
    LOG_INFO("generate_audio completed in %.2fs", (t1 - t0) * 1.0f / 1000);
    return true;
}

/* ------------------------- MiniMax Music 3 pipeline ------------------------ */

// k-th largest of the finite values in `window` (top-k threshold); -inf when
// top_k covers everything.
static float mm3_top_k_threshold(std::vector<float>& window, int64_t top_k) {
    if (top_k <= 0 || top_k >= (int64_t)window.size()) {
        return -std::numeric_limits<float>::infinity();
    }
    std::nth_element(window.begin(), window.begin() + (top_k - 1), window.end(), std::greater<float>());
    return window[top_k - 1];
}

// CFG + double top-k + softmax sampling over the compact semantic head
// (16384 codes + EOS). Returns the compact index; compact_size - 1 is EOS.
static int32_t mm3_sample_semantic_compact(const std::vector<float>& batch2_logits,
                                           int64_t compact_size,
                                           float guidance_scale,
                                           int64_t top_k,
                                           std::mt19937& rng,
                                           std::vector<float>& scratch_logits,
                                           std::vector<float>& scratch_window) {
    GGML_ASSERT((int64_t)batch2_logits.size() == 2 * compact_size);
    const float* cond   = batch2_logits.data();
    const float* uncond = batch2_logits.data() + compact_size;

    scratch_window.assign(cond, cond + compact_size);
    const float threshold = mm3_top_k_threshold(scratch_window, top_k);

    scratch_logits.assign((size_t)compact_size, -std::numeric_limits<float>::infinity());
    for (int64_t i = 0; i < compact_size; i++) {
        if (cond[i] >= threshold) {
            scratch_logits[i] = uncond[i] + (cond[i] - uncond[i]) * guidance_scale;
        }
    }
    scratch_window = scratch_logits;
    const float combined_threshold = mm3_top_k_threshold(scratch_window, top_k);

    std::vector<int32_t> candidates;
    std::vector<double> weights;
    float max_score = -std::numeric_limits<float>::infinity();
    for (int64_t i = 0; i < compact_size; i++) {
        if (std::isfinite(scratch_logits[i]) && scratch_logits[i] >= combined_threshold) {
            candidates.push_back((int32_t)i);
            max_score = std::max(max_score, scratch_logits[i]);
        }
    }
    if (candidates.empty() || !std::isfinite(max_score)) {
        LOG_ERROR("MiniMax Music 3 semantic sampler has no finite logits");
        return -1;
    }
    weights.reserve(candidates.size());
    for (int32_t idx : candidates) {
        weights.push_back(std::exp((double)(scratch_logits[idx] - max_score)));
    }
    std::discrete_distribution<size_t> distribution(weights.begin(), weights.end());
    return candidates[distribution(rng)];
}

// CFG + top-k + softmax over one depth codebook.
static int32_t mm3_sample_depth_code(const std::vector<float>& cond,
                                     ...
                                     std::vector<float>& scratch_logits,
                                     std::vector<float>& scratch_window) {
    const int64_t n = (int64_t)cond.size();
    scratch_logits.resize((size_t)n);
    ...
    float max_score = -std::numeric_limits<float>::infinity();
    for (int64_t i = 0; i < n; i++) {
        if (scratch_logits[i] >= threshold && std::isfinite(scratch_logits[i])) {
            candidates.push_back((int32_t)i);
            max_score = std::max(max_score, scratch_logits[i]);
        }
    }
    if (candidates.empty()) {
        LOG_ERROR("MiniMax Music 3 depth sampler has no finite logits");
        return -1;
    }
    weights.reserve(candidates.size());
    for (int32_t idx : candidates) {
...
// MiniMax Music 3 text2music. AR stage: the global LM decodes semantic codes
// at batch 2 (cond/uncond CFG); the depth decoder adds 7 residual codes per
// frame; per-frame conditioning = the LM hidden + the 7 depth hiddens.
// Denoise stage: per 200-frame chunk (hop 100), the condition encoder mixes
// the 8 hiddens, projects them and resamples to the latent rate; the flow
// transformer runs a plain Euler flow-matching loop with the null condition
// (zeros) as the CFG uncond branch, with an overlap-prompt carry between
// chunks; the vocoder decodes each chunk and the crops stitch them together.
static bool generate_minimax_music3_audio(sd_ctx_t* sd_ctx,
                                          const sd_vid_gen_params_t* sd_vid_gen_params,
                                          sd_audio_t** audio_out) {
    int64_t t0 = ggml_time_ms();
    ...
    auto conditioner = std::dynamic_pointer_cast<MiniMaxMusic3Conditioner>(sd_ctx->sd->cond_stage_model);
    auto flow        = std::dynamic_pointer_cast<MiniMaxMusic3::Music3FlowRunner>(sd_ctx->sd->diffusion_model);
    if (conditioner == nullptr || conditioner->ar == nullptr || flow == nullptr) {
        LOG_ERROR("MiniMax Music 3 runtime is incomplete (is the language model loaded via --llm?)");
        return false;
    }
    if (sd_ctx->sd->audio_vae_model == nullptr) {
        LOG_ERROR("no MiniMax Music 3 vocoder loaded; pass it with --vae");
        return false;
    }
    auto& ar            = *conditioner->ar;
    const auto& config  = ar.config;
    const int n_threads = sd_ctx->sd->n_threads;

    float duration = sd_vid_gen_params->audio_duration;
    if (duration <= 0.f) {
        duration = 20.f;
    }
    const int64_t target_frames = std::min<int64_t>(config.max_audio_frames,
                                                    (int64_t)(duration * (double)config.frame_rate));
    if (target_frames <= 0) {
        LOG_ERROR("MiniMax Music 3 audio duration is too short");
        return false;
    }
    ...
    float flow_cfg = sd_vid_gen_params->sample_params.guidance.txt_cfg;
    if (std::isnan(flow_cfg) || flow_cfg <= 0.f) {
        flow_cfg = 1.7f;
    }
    float ar_cfg = 1.5f;
    if (const char* env = getenv("SD_MM3_AR_CFG")) {
        ar_cfg = (float)atof(env);
    }
    int64_t top_k = 50;
    if (const char* env = getenv("SD_MM3_TOP_K")) {
        top_k = atoll(env);
    }
    std::string lyrics;
    if (sd_vid_gen_params->lyrics != nullptr) {
    ...
    if (lyrics.empty()) {
        if (const char* env = getenv("SD_MM3_LYRICS")) {
            lyrics = env;
        }
    }
    if (!request.negative_prompt.empty()) {
        LOG_WARN("MiniMax Music 3's unconditional branch is fixed; the negative prompt is ignored");
    }

    LOG_INFO("generate_audio: %.1fs -> %" PRId64 " AR frames, %d flow steps, flow cfg %.2f, ar cfg %.2f, top_k %" PRId64,
             duration, target_frames, sample_steps, flow_cfg, ar_cfg, top_k);
    if (lyrics.empty()) {
        LOG_INFO("no lyrics given (pass --lyrics to sing); generating from the caption alone");
    }

    std::vector<int32_t> cond_ids;
    std::vector<int32_t> uncond_ids;
    if (!conditioner->build_prompt_ids(request.prompt, lyrics, cond_ids, uncond_ids)) {
        LOG_ERROR("MiniMax Music 3 prompt encoding failed");
        return false;
    }
    LOG_DEBUG("mm3 prompt: %zu tokens", cond_ids.size());

    /* ------------------------------- AR stage ------------------------------ */
    const int64_t hidden_frame_width = config.condition_layers * config.hidden_size;
    std::vector<float> frame_hiddens;
    frame_hiddens.reserve((size_t)(target_frames * hidden_frame_width));

    std::mt19937 sample_rng((uint32_t)request.seed);
    std::vector<float> scratch_logits;
    std::vector<float> scratch_window;
    const int64_t compact_size = config.semantic_vocab_size + 1;

    int64_t ar_start = ggml_time_ms();
    {
        MiniMaxMusic3::Music3ArStepOutput step;
        if (!ar.prefill(n_threads, cond_ids, uncond_ids, step)) {
            LOG_ERROR("MiniMax Music 3 prefill failed");
            return false;
        }

        std::vector<float> depth_cond_hidden;
        std::vector<float> depth_cond_logits;
        std::vector<float> depth_uncond_logits;
        std::vector<float> feedback;
        std::vector<int32_t> codes((size_t)config.depth_codebooks, 0);

        for (int64_t frame = 0; frame <= target_frames; frame++) {
            if (sd_ctx->sd->get_cancel_flag() == SD_CANCEL_ALL) {
                LOG_ERROR("cancelling generation during the AR stage");
                return false;
            }
            const int32_t compact = mm3_sample_semantic_compact(step.logits, compact_size,
                                                                ar_cfg, top_k, sample_rng,
                                                                scratch_logits, scratch_window);
            if (compact < 0) {
                return false;
            }
            if (compact == compact_size - 1) {  // EOS
                LOG_INFO("mm3 AR hit the end token at frame %" PRId64, frame);
                break;
            }
            const int32_t semantic_code = compact;

            // depth decoder: 7 residual codebooks, each conditioned on the
            // codes so far; collect the cond hiddens along the way
            codes.assign(1, semantic_code);
            std::vector<float> frame_depth_hiddens;
            frame_depth_hiddens.reserve((size_t)((config.depth_codebooks - 1) * config.hidden_size));
            bool depth_ok = true;
            for (int64_t codebook = 1; codebook < config.depth_codebooks; codebook++) {
                std::vector<int32_t> residual_codes(codes.begin() + 1, codes.end());
                if (!ar.depth_step(n_threads, step.cond_hidden, step.uncond_hidden,
                                   semantic_code, residual_codes, codebook,
                                   depth_cond_hidden, depth_cond_logits, depth_uncond_logits)) {
                    depth_ok = false;
                    break;
                }
                const int32_t code = mm3_sample_depth_code(depth_cond_logits, depth_uncond_logits,
                                                           ar_cfg, top_k, sample_rng,
                                                           scratch_logits, scratch_window);
                if (code < 0) {
                    depth_ok = false;
                ...
                frame_depth_hiddens.insert(frame_depth_hiddens.end(),
                                           depth_cond_hidden.begin(), depth_cond_hidden.end());
            }
            if (!depth_ok) {
                LOG_ERROR("MiniMax Music 3 depth decoding failed");
                return false;
            }

            if (frame > 0) {
                frame_hiddens.insert(frame_hiddens.end(), step.cond_hidden.begin(), step.cond_hidden.end());
                frame_hiddens.insert(frame_hiddens.end(), frame_depth_hiddens.begin(), frame_depth_hiddens.end());
            }
            if (frame == target_frames) {
                break;
            }
            if (!ar.feedback_embedding(n_threads, codes, feedback)) {
                LOG_ERROR("MiniMax Music 3 feedback embedding failed");
                return false;
            }
            if (!ar.decode(n_threads, feedback, step)) {
                LOG_ERROR("MiniMax Music 3 LM decode failed");
                return false;
            }
            if ((frame + 1) % 50 == 0) {
                LOG_INFO("mm3 AR frame %" PRId64 "/%" PRId64 " (%.2fs elapsed)",
                         frame + 1, target_frames, (ggml_time_ms() - ar_start) / 1000.f);
            }
        }
    }
    const int64_t generated_frames = (int64_t)frame_hiddens.size() / hidden_frame_width;
    if (generated_frames <= 0) {
        LOG_ERROR("MiniMax Music 3 AR produced no frames");
        return false;
    }
    LOG_INFO("mm3 AR stage: %" PRId64 " frames in %.2fs",
             generated_frames, (ggml_time_ms() - ar_start) / 1000.f);

    // the LM (and its KV cache) is done; get its memory back before the flow
    conditioner->runner_done();
    sd_ctx->sd->evict_text_encoder_params();

    /* ------------------------- chunk geometry & carry ----------------------- */
    const int64_t chunk_frames = config.chunk_frames;
    const int64_t hop_frames   = config.chunk_hop_frames;
    auto predicted_latents     = [&](int64_t frames) -> int64_t {
        return (int64_t)((double)frames * config.condition_ratio);
    };
    const int64_t chunk_latents   = predicted_latents(chunk_frames);
    const int64_t kept_latents    = predicted_latents(hop_frames);
    ...
    if (generated_frames <= chunk_frames) {
        starts.push_back(0);
    } else {
        for (int64_t start = 0; start < generated_frames - hop_frames; start += hop_frames) {
            starts.push_back(start);
        }
    }

    const int64_t C  = flow->config.flow_in_channels;
    const int64_t CD = flow->config.condition_dim;

    // sigmas: t goes 0 -> 1, velocity Euler
    std::vector<float> sigmas((size_t)sample_steps + 1);
    if (sample_steps == 1) {
        sigmas[0] = 0.0f;
    } else {
        const float end = 1.0f / (float)sample_steps;
        for (int i = 0; i < sample_steps; i++) {
            const float raw = 1.0f + (end - 1.0f) * (float)i / (float)(sample_steps - 1);
            sigmas[(size_t)i] = 1.0f - raw;
        }
    }
    sigmas[(size_t)sample_steps] = 1.0f;

    struct DenoisedChunk {
        std::vector<float> latents;  // [C * latent_frames], channel-major
        int64_t latent_frames = 0;
    };
    std::vector<DenoisedChunk> denoised;
    denoised.reserve(starts.size());

    std::vector<float> previous_latent;     // [C * overlap_latents]
    std::vector<float> previous_condition;  // [CD * overlap_latents]

    int64_t flow_start = ggml_time_ms();
    for (size_t chunk_index = 0; chunk_index < starts.size(); chunk_index++) {
        if (sd_ctx->sd->get_cancel_flag() == SD_CANCEL_ALL) {
            LOG_ERROR("cancelling generation during the flow stage");
            return false;
        }
        const int64_t start       = starts[chunk_index];
        const int64_t end         = std::min(start + chunk_frames, generated_frames);
        const int64_t frame_count = end - start;

        int64_t condF = 0;
        std::vector<float> condition;
        if (!flow->encode_condition(n_threads,
                                    frame_hiddens.data() + start * hidden_frame_width,
                                    frame_count, condF, condition)) {
            LOG_ERROR("MiniMax Music 3 condition encoding failed");
            return false;
        }

        int64_t overlap = 0;
        if (!previous_latent.empty()) {
            overlap = std::min<int64_t>((int64_t)previous_latent.size() / C, condF);
            // splice the carried condition window over this chunk's prefix
            for (int64_t c = 0; c < CD; c++) {
                std::copy(previous_condition.begin() + c * overlap,
                          previous_condition.begin() + (c + 1) * overlap,
                          condition.begin() + c * condF);
            }
        }

        auto noise = sd::Tensor<float>::randn({condF, C, 1}, sd_ctx->sd->rng);
        std::vector<float> latents(noise.data(), noise.data() + noise.numel());
        std::vector<float> noise_prompt((size_t)(C * overlap));
        for (int64_t c = 0; c < C; c++) {
            std::copy(latents.begin() + c * condF,
                      latents.begin() + c * condF + overlap,
                      noise_prompt.begin() + c * overlap);
        }

        std::vector<float> velocity;
        for (int step = 0; step < sample_steps; step++) {
            const float t      = sigmas[(size_t)step];
            const float t_next = sigmas[(size_t)step + 1];
            if (overlap > 0) {
                // overlap prompting: the prefix rides the flow from noise to
                // the previous chunk's latents
                for (int64_t c = 0; c < C; c++) {
                    for (int64_t f = 0; f < overlap; f++) {
                        const float n = noise_prompt[(size_t)(c * overlap + f)];
                        const float p = previous_latent[(size_t)(c * overlap + f)];
                        latents[(size_t)(c * condF + f)] = (1.0f - (1.0f - 1.0e-6f) * t) * n + t * p;
                    }
                }
            }
            if (!flow->predict_velocity(n_threads, latents, condition, condF, t, 2, velocity)) {
                LOG_ERROR("MiniMax Music 3 flow step failed");
                return false;
            }
            const size_t branch = (size_t)(C * condF);
            const float dt      = t_next - t;
            for (size_t i = 0; i < branch; i++) {
                const float v = velocity[branch + i] + (velocity[i] - velocity[branch + i]) * flow_cfg;
                latents[i] += dt * v;
            }
            if (sd_ctx->sd->get_cancel_flag() == SD_CANCEL_ALL) {
                LOG_ERROR("cancelling generation during the flow stage");
        ...
        if (overlap > 0) {
            for (int64_t c = 0; c < C; c++) {
                std::copy(previous_latent.begin() + c * overlap,
                          previous_latent.begin() + (c + 1) * overlap,
                          latents.begin() + c * condF);
            }
        }

        // carry the second-to-last overlap window into the next chunk
        const int64_t carry_start = std::max<int64_t>(0, condF - 2 * overlap_latents);
        const int64_t carry_end   = std::max(carry_start, condF - overlap_latents);
        const int64_t carry_len   = carry_end - carry_start;
        previous_latent.assign((size_t)(C * carry_len), 0.0f);
        previous_condition.assign((size_t)(CD * carry_len), 0.0f);
        for (int64_t c = 0; c < C; c++) {
            std::copy(latents.begin() + c * condF + carry_start,
                      latents.begin() + c * condF + carry_end,
                      previous_latent.begin() + c * carry_len);
        }
        for (int64_t c = 0; c < CD; c++) {
            std::copy(condition.begin() + c * condF + carry_start,
                      condition.begin() + c * condF + carry_end,
                      previous_condition.begin() + c * carry_len);
        }

        LOG_INFO("mm3 flow chunk %zu/%zu done (%.2fs elapsed)",
                 chunk_index + 1, starts.size(), (ggml_time_ms() - flow_start) / 1000.f);
        denoised.push_back({std::move(latents), condF});
    }
    flow->runner_done();

    /* -------------------------------- vocoder ------------------------------- */
    const int64_t hop = config.vocoder_hop_length;
    ...
    for (size_t chunk_index = 0; chunk_index < denoised.size(); chunk_index++) {
        if (sd_ctx->sd->get_cancel_flag() == SD_CANCEL_ALL) {
            LOG_ERROR("cancelling generation during the vocoder stage");
            return false;
        }
        auto& chunk = denoised[chunk_index];
        sd::Tensor<float> latent_tensor({chunk.latent_frames, C}, std::move(chunk.latents));
        auto audio = sd_ctx->sd->audio_vae_model->decode(n_threads, latent_tensor);
        if (audio.empty() || audio.dim() < 2 || audio.shape()[1] < 2) {
            LOG_ERROR("MiniMax Music 3 vocoder decode failed");
            return false;
        }
        const int64_t samples = audio.shape()[0];
        const int64_t left    = chunk_index == 0 ? 0 : crop_left * hop;
        const int64_t right   = chunk_index + 1 == denoised.size() ? 0 : crop_right * hop;
        const int64_t begin   = std::min(left, samples);
        const int64_t finish  = std::max(begin, samples - right);
        const float* data     = audio.data();  // [samples, 2] planar
        left_samples.insert(left_samples.end(), data + begin, data + finish);
        right_samples.insert(right_samples.end(), data + samples + begin, data + samples + finish);
    }

    const int64_t total_samples = (int64_t)left_samples.size();
    if (total_samples <= 0) {
        LOG_ERROR("MiniMax Music 3 produced no audio");
        return false;
    }
    sd::Tensor<float> waveform({total_samples, 2, 1, 1});
    for (int64_t i = 0; i < total_samples; i++) {
        waveform.data()[i]                 = std::clamp(left_samples[(size_t)i], -1.0f, 1.0f);
        waveform.data()[total_samples + i] = std::clamp(right_samples[(size_t)i], -1.0f, 1.0f);
    }
    ...
    sd_ctx->sd->lora_stat();
    int64_t t1 = ggml_time_ms();
    LOG_INFO("generate_audio completed in %.2fs (%.1fs of audio)",
             (t1 - t0) * 1.0f / 1000, (double)total_samples / config.sample_rate);
    return true;
}
...
SD_API bool generate_video(sd_ctx_t* sd_ctx,
    ...
    if (sd_version_is_minimax_music3(sd_ctx->sd->version)) {
        return generate_minimax_music3_audio(sd_ctx, sd_vid_gen_params, audio_out);
    }
    if (sd_version_supports_audio_generation(sd_ctx->sd->version)) {
        return generate_ace_audio(sd_ctx, sd_vid_gen_params, audio_out);
    }
    ...
    // Same reasoning as the image path: conditioning is done, and a video's
    // denoising loop is the longest one there is to be paging through.
    sd_ctx->sd->evict_text_encoder_params();
    if (latent_upscale_enabled) {
